...
function addGamesToPage(games) {
    ...
    for (var i = 0; i < games.length; i++){      
        ...
        const newContent = `<img src="${obj["img"]}" class="game-img">
        <h1>${obj["name"]}</h1>
        <p>${obj["description"]}</p>
        <p>Backers: ${obj["backers"]}</p>
        `;
        newDiv.innerHTML = newContent;
        var parentDiv = document.getElementById("games-container");
...
const totalContrubutions = GAMES_JSON.reduce((acc, game) =>{ return acc + game["backers"]}, 0); 
contributionsCard.innerText = totalContrubutions.toLocaleString('en-US');
...
function filterUnfundedOnly() {
    ...
    const newDiv = document.createElement("div");
    newDiv.setAttribute("id", "games-container");
...
let explainsUnfunded = `A total of $ ${totalPledged.toLocaleString('en-US')} has been raised for ${gamesNum} games. Currently, ${unfundedNum} ${ unfundedNum > 1 ? "games" :"game"} remain unfunded, We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
let newP = document.createElement("p");
newP.innerText = explainsUnfunded;
